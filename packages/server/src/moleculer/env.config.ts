...
process.env.B6_TRANSPORTER = process.env.B6_TRANSPORTER || process.env.STEEDOS_TRANSPORTER || process.env.TRANSPORTER;
...
function parseEnvToJSON(env, prefixes) {
    ...
    Object.keys(env).forEach((key) => {
        ...
        // 去除前缀并转为小写
        const strippedKey = key.replace(new RegExp(`^(${prefixes.join('|')})`), '').toLowerCase();
...
export default () => {
    ...
    const config = global.b6Server?.config || {};
    return {
        ...config,
        ...env,
    }
